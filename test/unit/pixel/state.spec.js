...
describe('EventComposition', () => {
  ...
  it('should construct valid tuples for valid members', function () {
    const pixelData = { appId: '9898' }
    const event = new StateWrapper(pixelData)
    assert.includeDeepMembers(event.asTuples(params), [['aid', '9898']])
  })

  it('should construct valid params for valid members', function () {
    const pixelData = { appId: '9898' }
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql('?aid=9898')
  })
  ...
  it('should base64 the source', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?aid=9898&se=${b64EncodedEventSource}`)
    assert.includeDeepMembers(event.asTuples(params), [['aid', '9898'], ['se', b64EncodedEventSource]])
  })
  ...
  it('should send the usPrivacyString', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql('?us_privacy=1---')
    assert.includeDeepMembers(event.asTuples(params), [['us_privacy', '1---']])
  })
  ...
  it('should send the legacyId', function () {
    ...
    const b64EncodedEventSource = 'eyJldmVudE5hbWUiOiJ2aWV3Q29udGVudCJ9'
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?aid=9898&se=${b64EncodedEventSource}&lduid=${legacyDuid}`)
    assert.includeDeepMembers(event.asTuples(params), [['aid', '9898'], ['se', b64EncodedEventSource], ['lduid', legacyDuid]])
  })
  ...
  it('should send the gdprApplies & gdprConsent', function () {
    ...
    const event = new StateWrapper(pixelData)
    const b64EncodedEventSource = 'eyJldmVudE5hbWUiOiJ2aWV3Q29udGVudCJ9'
    expect(event.asQueryString(params)).to.eql(`?se=${b64EncodedEventSource}&gdpr=1&gdpr_consent=some-string`)
    assert.includeDeepMembers(event.asTuples(params), [['se', b64EncodedEventSource], ['gdpr', '1'], ['gdpr_consent', 'some-string']])
  })
  ...
  it('should send the gdprApplies as 0 if false', function () {
    ...
    const event = new StateWrapper(pixelData)
    const b64EncodedEventSource = 'eyJldmVudE5hbWUiOiJ2aWV3Q29udGVudCJ9'
    expect(event.asQueryString(params)).to.eql(`?se=${b64EncodedEventSource}&gdpr=0&gdpr_consent=some-string`)
    assert.includeDeepMembers(event.asTuples(params), [['se', b64EncodedEventSource], ['gdpr', '0'], ['gdpr_consent', 'some-string']])
  })
  ...
  it('should send the tracker name', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?tna=${trackerName}`)
    assert.includeDeepMembers(event.asTuples(params), [['tna', trackerName]])
  })
  ...
  it('should send the page url', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?pu=${encodeURIComponent(pageUrl)}`)
    assert.includeDeepMembers(event.asTuples(params), [['pu', encodeURIComponent(pageUrl)]])
  })
  ...
  it('should send the application error', function () {
    ...
    expect(event.asQueryString(params)).to.eql(`?ae=${b64EncodedEventSource}`)
    assert.includeDeepMembers(event.asTuples(params), [['ae', b64EncodedEventSource]])
  })
  ...
  it('should update the data', function () {
    ...
    expect(newEvent.asQueryString(params)).to.eql(`?aid=9898&se=${b64EncodedEventSource}&duid=213245`)
    assert.includeDeepMembers(newEvent.asTuples(params), [['aid', '9898'], ['se', b64EncodedEventSource], ['duid', '213245']])
  ...
  it('should send the provided email hash', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?aid=9898&se=${b64EncodedEventSource}&e=e168e0eda11f4fbb8fbd7cfe5f750cd0f7e7f4d8649da68e073e927504ec5d72`)
    assert.includeDeepMembers(event.asTuples(params), [['aid', '9898'], ['se', b64EncodedEventSource], ['e', 'e168e0eda11f4fbb8fbd7cfe5f750cd0f7e7f4d8649da68e073e927504ec5d72']])
  })
  ...
  it('should never send emails as plain text, and hash the email that is set in the source', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql(`?aid=9898&se=${b64EncodedEventSource}&e=${hashes.md5}%2C${hashes.sha1}%2C${hashes.sha256}`)
    assert.includeDeepMembers(event.asTuples(params), [['aid', '9898'], ['se', b64EncodedEventSource]], ['e', [`${hashes.md5},${hashes.sha1},${hashes.sha256}`]])
  })
  ...
  it('should send the retrieved identifiers', function () {
    ...
    expect(event.asQueryString(params)).to.eql(`?ext_${cookie1.name}=${cookie1.value}&ext_${cookie2.name}=${cookie2.value}`)
    assert.includeDeepMembers(event.asTuples(params), [
  ...
  it('should send the hashes found in retrieved identifiers', function () {
    ...
    expect(event.asQueryString(params)).to.eql(`?scre=${hashes1.md5},${hashes1.sha1},${hashes1.sha256}&scre=${hashes2.md5},${hashes2.sha1},${hashes2.sha256}`)
    assert.includeDeepMembers(event.asTuples(params), [
  ...
  it('should send decisionIds ', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql('?li_did=1%2C2')
    assert.includeDeepMembers(event.asTuples(params), [['li_did', '1%2C2']])
  })
  ...
  it('should not send decisionIds if array is empty', function () {
    ...
    const event = new StateWrapper(pixelData)
    expect(event.asQueryString(params)).to.eql('')
    assert.includeDeepMembers(event.asTuples(params), [])
  })
  ...
  it('should limit the number of items', function () {
    ...
    expect(eventWithItems.asQueryString(params)).to.eql('?se=eyJpdGVtcyI6WzAsMSwyLDMsNCw1LDYsNyw4LDldfQ')
    assert.includeDeepMembers(eventWithItems.asTuples(params), [['se', 'eyJpdGVtcyI6WzAsMSwyLDMsNCw1LDYsNyw4LDldfQ']])
