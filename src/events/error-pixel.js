...
export function register (state, callHandler) {
  try {
    ...
    if (window && window[C.EVENT_BUS_NAMESPACE] && window[C.EVENT_BUS_NAMESPACE].child && isFunction(window[C.EVENT_BUS_NAMESPACE].child.on)) {
      window[C.EVENT_BUS_NAMESPACE].child.on(C.ERRORS_PREFIX, _pixelError)
    }
