...
function StorageHandler$1 (storageStrategy, externalStorageHandler) {
  ...
  const _orElseNoOp = (fName) => strEqualsIgnoreCase(storageStrategy, StorageStrategy.none) ? _noOp$2 : _externalOrError(fName);
  const handler = {
    localStorageIsEnabled: _orElseNoOp('localStorageIsEnabled'),
    getCookie: _externalOrError('getCookie'),
    getDataFromLocalStorage: _externalOrError('getDataFromLocalStorage')
...
function _minimalInitialization (liveConnectConfig, externalStorageHandler, externalCallHandler) {
  try {
    const callHandler = CallHandler(externalCallHandler);
    const storageHandler = StorageHandler$1(liveConnectConfig.storageStrategy, externalStorageHandler);
    const peopleVerifiedData = merge(liveConnectConfig, enrich$3(liveConnectConfig, storageHandler));
    const finalData = merge(peopleVerifiedData, enrich$4(peopleVerifiedData, storageHandler));
    const resolver = IdentityResolver$1(finalData, storageHandler, callHandler);
    return {
