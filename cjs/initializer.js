...
function md5 (t) {
  ...
  for (o$1 = h = 0; o$1 < t; o$1 += 16) {
    for (u = a; h < 64; u = [n = u[3], e + ((n = u[0] + [e & f | ~e & n, n & e | ~n & f, e ^ f ^ n, f ^ (e | ~n)][u = h >> 4] + r$1[h] + ~~c[o$1 | 15 & [h, 5 * h + 1, 3 * h + 5, 7 * h][u]]) << (u = [7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21][4 * u + h++ % 4]) | n >>> -u), e, f]) {
      e = 0 | u[1], f = u[2];
    }
    for (h = 4; h;) {
      a[--h] += u[h];
...
function sha256 (u) {
  ...
  for (p = []; o < u; o += 16) {
    for (e = c.slice(); r < 64; e.unshift(a + (f(a = e[0], 2) ^ f(a, 13) ^ f(a, 22)) + (a & e[1] ^ e[1] & e[2] ^ e[2] & a))) {
      e[3] += a = 0 | (p[r] = r < 16 ? ~~i[r + o] : (f(a = p[r - 2], 17) ^ f(a, 19) ^ a >>> 10) + p[r - 7] + (f(a = p[r - 15], 7) ^ f(a, 18) ^ a >>> 3) + p[r - 16]) + e.pop() + (f(a = e[4], 6) ^ f(a, 11) ^ f(a, 25)) + (a & e[5] ^ ~a & e[6]) + n[r++];
    }
    for (r = 8; r;) {
      c[--r] += e[r];
    }
...
function _pixelError(error) {
  if (_pixelSender) {
    _pixelSender.sendPixel(new StateWrapper(asErrorDetails(error)).combineWith(_state || {}).combineWith(enrich$4({})));
  }
}
...
function _standardInitialization(liveConnectConfig, externalStorageHandler, externalCallHandler) {
  try {
    ...
    var resolver = IdentityResolver$1(postManagedState.data, storageHandler, callHandler);
...
function _minimalInitialization(liveConnectConfig, externalStorageHandler, externalCallHandler) {
  try {
    ...
    var peopleVerifiedData = merge(configWithPrivacy, enrich$1(configWithPrivacy, storageHandler));
    var peopleVerifiedDataWithAdditionalIds = merge(peopleVerifiedData, enrich(peopleVerifiedData, storageHandler));
