...
var _additionalParams$1 = function _additionalParams(params) {
  if (params && isObject(params)) {
    var array = [];
    Object.keys(params).forEach(function (key) {
      var value = params[key];
...
function _asParamOrEmpty$2(param, value, transform) {
  if (isNonEmpty(value)) {
    return [param, transform(value)];
  } else {
    return [];
...
function IdentityResolver$1(config, storageHandler, calls) {
  try {
    ...
    var timeout = idexConfig.ajaxTimeout || DEFAULT_IDEX_AJAX_TIMEOUT;
    var tuples = [];
    tuples.push(_asParamOrEmpty$2('duid', nonNullConfig.peopleVerifiedId, encodeURIComponent));
    tuples.push(_asParamOrEmpty$2('us_privacy', nonNullConfig.usPrivacyString, encodeURIComponent));
    tuples.push(_asParamOrEmpty$2('gdpr', nonNullConfig.gdprApplies, function (v) {
      return encodeURIComponent(v ? 1 : 0);
    }));
    tuples.push(_asParamOrEmpty$2('gdpr_consent', nonNullConfig.gdprConsent, encodeURIComponent));
    externalIds.forEach(function (retrievedIdentifier) {
      tuples.push(_asParamOrEmpty$2(retrievedIdentifier.name, retrievedIdentifier.value, encodeURIComponent));
    });
    var composeUrl = function composeUrl(additionalParams) {
      var originalParams = tuples.slice().concat(_additionalParams$1(additionalParams));
      var params = toParams(originalParams);
      return "".concat(url, "/").concat(source, "/").concat(publisherId).concat(params);
    };
    var unsafeResolve = function unsafeResolve(successCallback, errorCallback, additionalParams) {
...
function _minimalInitialization(liveConnectConfig, externalStorageHandler, externalCallHandler) {
  try {
    ...
    var peopleVerifiedData = _objectSpread2(_objectSpread2({}, liveConnectConfig), enrich$3(liveConnectConfig, storageHandler));
